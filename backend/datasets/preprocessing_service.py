...
class PreprocessingService:
    ...
    def normalize_numeric_columns(self, df: pd.DataFrame, scaler_type: str = 'standard') -> pd.DataFrame:
        """Normalize numeric columns using StandardScaler, MinMaxScaler, or RobustScaler."""
        df = df.copy()
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
        if not numeric_cols:
            return df
        scaler_cls = self.scalers.get(scaler_type, StandardScaler)
        scaler = scaler_cls()
    ...
    def save_processed_dataset(self, df: pd.DataFrame, output_path: str) -> str:
        """Save processed DataFrame to CSV. Returns the saved path."""
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        df.to_csv(output_path, index=False)
        return output_path

    def run_full_preprocessing(self, file_path: str) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        Full pipeline: load -> handle missing -> remove duplicates -> normalize -> encode -> return.
        Does NOT save; caller should use save_processed_dataset.
        """
        ...
        summary = {
            ...
            'afterRows': len(df),
            'beforeCols': before_cols,
            'afterCols': len(df.columns),
            'duplicatesRemoved': before_rows - len(df),
        }
        return df, summary
